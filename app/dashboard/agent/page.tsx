...
export default function AgentPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (cancelled) return;
      if (loadErr) {
        console.error("Failed to load conversation:", loadErr.message);
        setMessages([]);
      } else {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      <div className="ml-20 transition-all duration-300">
        {!hasMessages ? (
          <main className="mx-auto flex w-full max-w-3xl flex-col items-center px-4 pt-12 pb-16">
            <div className="mb-8 flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-medium text-slate-600 shadow-sm">
